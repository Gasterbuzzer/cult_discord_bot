...
def get_fullmoon(month=None):
	...
	if month == None:
		...
		month_short_correct = re.findall(rf'<tr id="{month_text}"   >.*?<td>(.*?)<span class="NoMo">', page_text, flags=re.DOTALL)
		#print(f"Month Short {month_short_correct}")

		# {month_text} Exmaple output: september
		# We find the perfect day of the given month.
		fullmoon_date = re.findall(rf'<tr id="{month_text}"   >.*<td>{month_short_correct[0]} (.*?), {year_number}<br>..... ...</td>', page_text, flags=re.DOTALL)
